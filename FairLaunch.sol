...
contract FairLaunch is Ownable {
    ...
    uint256 private constant MAX_FOR_SALE = 500000e18; // 500k

    uint256 private constant VESTING_TERM = 14 days;

    uint256 private constant EXCHANGE_RATE = 10; // 10 asset -> 1 POWER
    ...
    function payFor(uint256 _amount) public pure returns (uint256 POWERAmount_) {
        // POWER decimals: 9
        // asset decimals: 18
        POWERAmount_ = _amount.mul(1e9).div(EXCHANGE_RATE).div(1e18);
    }
    ...
    function _stakeOrSend(
        ...
        if (!_stake) {
            ...
            POWER.transfer(_recipient, _amount); // send payout
        } else {
            // if user wants to stake
            POWER.approve(staking, _amount);
    ...
    function finalize() external onlyOwner {
        require(!finalized, "already finalized");
        require(address(POWER) != address(0), "0 addr: POWER");
        ...
        POWER.approve((router), 0);
        POWER.approve((router), mintForLiquidity);
        ...
        // give treasury 100% LP, mint 50% POWER
        // FAIR DEAL !!!!
        address liquidityPair = IPancakeFactory(factory).getPair(address(BUSD), address(POWER));
